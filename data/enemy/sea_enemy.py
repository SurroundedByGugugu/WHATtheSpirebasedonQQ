...
class CorsoalEnemy(PatternEnemy):
    def __init__(self):
        intent_cycle = [
            ...
            EnemyIntent(kind="status", target="self", status="dexterity", value=4),
        ]
...
class MareanieEnemy(Enemy):
    ...
    def _get_opening_intent(self):
        if self._locked_opening_intent is None:
            self._locked_opening_intent = random.choice([
                ...
                EnemyIntent(kind="status", target="self", status="poison_thorns", value=2),
            ])
    ...
    def get_current_intent(self):
        ...
        if self._intent_index == 1:
            return EnemyIntent(kind="attack", value=4, target="corsoal_or_player")
    ...
    def act(self):
        ...
        if self._intent_index == 1:
            ...
            logs.append("{} 准备优先对珊瑚施加 4 层中毒，并造成 4 点伤害。".format(self.name))
            self.advance_intent()
            return EnemyActionResult(action=action, logs=logs)
